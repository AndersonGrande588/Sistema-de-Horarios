...
- **Propósito:** visualizar de un vistazo qué técnico trabaja en cada tienda, en qué días y con qué tipo de jornada (normal o guardia), y así coordinar cobertura de las tiendas.
...
3. El usuario elige una **tienda** → se muestra el banner de la tienda + el **calendario mensual** del personal.
4. El calendario se puede filtrar por **mes/año**, por **tipo de jornada** (Todas / Jornada normal / Guardia) y muestra el **número de día** real de cada celda.
...
| Archivo | Descripción |
|---|---|
| `index.html` | Interfaz principal (selectores, banner, controles y contenedor del calendario). |
| `styles.css` | Todas las reglas de estilo (hoja única). |
| `funciones.js` | Lógica completa: carga de datos, selección país/tienda, filtros y render del calendario. |
...
const countries = {
    ...
    // ... (mexico, argentina, chile, peru, ecuador)
};
...
Actualmente **8 tiendas** tienen horarios (≈170 registros), todos con `"month": "Septiembre"`. La tienda 9 (`Bima`, Panamá) existe pero no tiene horarios cargados.

> **Nota:** los datos no incluyen el número del día del mes; el calendario **calcula** los números de cada celda a partir de semana + día usando el calendario real del mes/año seleccionado.
...
- Estado global:
  ...
  - `filterMode`: señalización activa (`todas` | `normal` | `guardia`).
  - `currentMonth` / `currentYear`: mes y año del filtro.
...
- `MONTH_NAMES`: lista ordenada de los 12 meses en español (usada para el filtro y para numerar días).
...
- `renderStore()`: se ejecuta al elegir tienda. Muestra el banner (nombre, dirección, país, nº de empleados), reinicia el modo de señalización a "Todas", prepara los filtros de fecha (`populateDateFilters`) y dibuja el calendario (`renderCalendar`). Si se deselecciona, oculta todo.

### BLOQUE 3 — Botones de señalización

- `selectMode(mode)`: cambia `filterMode` y redibuja el calendario.
- `updateButtons()`: marca el botón activo de la barra Todas/Jornada normal/Guardia.

### BLOQUE 3.5 — Filtros de mes y año

- `populateDateFilters()`: llena el selector de **mes** con los 12 meses y el de **año** con los años presentes en los datos de la tienda (o el año actual si no hay). Preselecciona el **mes actual del sistema** y el primer año. Hace visible la fila de filtros.
- `selectMonth()` / `selectYear()`: al cambiar el valor, **limpian la información previa** (`staffList = ""`) y vuelven a renderizar el calendario.

### BLOQUE 4 — Calendario mensual

- `renderCalendar()`: función principal del render.
  1. Filtra los registros de la tienda por mes y año.
  ...
  4. Calcula el **número de día real** de cada celda:
     - `firstWeekday` → día de la semana del día 1 del mes (lunes = 0).
     - `dayNum = (semanaIndex * 7 + díaColumna - firstWeekday) + 1`.
     - Los números fuera del rango del mes (antes del 1 o después del último) se muestran en gris (`out-month`).
- Helpers internos:
  ...
  - `turnoColor(sch)`: decide el color del evento (guardia → naranja; duración ≥ 540 min → azul/completa; si no → verde/parcial).
  - `toMin(t)`: convierte `"HH:MM"` a minutos.
  - `visible(sch)`: filtra por modo de señalización.

### Render de un turno (evento)

Cada turno se dibuja como `.cal-event` con:
- **foto** del técnico (o iniciales si no hay foto),
- **nombre** (una línea, corta con `…` si es largo),
- **hora** (rango `HH:MM – HH:MM`) o la etiqueta **"Guardia"** si es tipo guardia.
- `title` (tooltip) con nombre + hora.
...
| `.card`, `.selector-card` | Tarjetas de los selectores. |
...
| `.filter-row`, `.filter-group` | Fila de filtros (mes/año). |
...
| `.mode-bar`, `.mode-btn` | Barra y botones de señalización. Botón activo en azul. |
...
- **España de tiendas con datos:** solo 8 de las 9 tiendas definidas tienen horarios.
...
- ✅ Calendario mensual en rejilla (semana × día) con numeración de días.
- ✅ Botones de señalización (Todas / Jornada normal / Guardia) en lugar de desplegables.
- ✅ Filtros de **mes** (12 meses) y **año** con limpieza de la información al cambiar.
- ✅ Etiqueta "Guardia" en los turnos de guardia (sin horario).
- ✅ Celdas con altura fija uniforme (96px) y scroll interno para días muy cargados.
- ✅ Alturas/anchuras del calendario corregidas (se removió el CSS obsoleto de la vista semanal).